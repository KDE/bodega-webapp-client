...
var doWork = function(req, res) {
    ...
    async.parallel([
        ...
        function(callback) {
            ...
            app.BodegaManager.connect(options('collection/list', null, true), function(remoteRes, chunk) {
                var reply = JSON.parse(chunk.toString('utf8'), 'utf8');

                function next() {
                    var collectionReply = reply.collections.shift();

                    if (collectionReply === undefined) {
                        callback(null, undefined);
                        return;
                    }

                    var temp = {
                        name: collectionReply.name,
                        id: collectionReply.id,
                        isCollection: true
                    };

                    //we have to find out how many assets this collection has in order
                    //to find out how many pages we need
                    app.BodegaManager.connect(options('collection/list/' + temp.id, null, true), function(remRes, data) {
                        var jsonData = JSON.parse(data.toString('utf8'), 'utf8');
                        var collectionAssetCount = 0;
                        console.log(jsonData)
                        for (var j in jsonData.collection.assets) {
                            collectionAssetCount++;
                        }
                        temp.pages = findPages(collectionAssetCount);
...
sockjs_echo.on('connection', function(conn) {
    conn.on('data', function(data) {
        ...
        if (isCollection) {
            app.BodegaManager.connect(options('collection/list/' + categoryId, collectionQuery, true), function(remRes, data) {
                var jsonData = JSON.parse(data.toString('utf8'), 'utf8');
                parseAssetsFromJson(jsonData.collection.assets, function(assetsResult) {
            ...
            app.BodegaManager.connect(options('channel/' + categoryId, channelQuery, true), function(remRes, data) {
                var jsonData = JSON.parse(data.toString('utf8'), 'utf8');
                parseAssetsFromJson(jsonData.assets, function(assetsResult) {
                    conn.write(assetsResult);
                });
            });
        }
...
var parseAssetsFromJson = function(json, cb) {
    var assetArray = [];
    for (var assetIt in json) {
        //take the assets for the jade
        var asset = {
            id: json[assetIt].id,
            name: json[assetIt].name,
            version: json[assetIt].version,
            license: json[assetIt].license,
            author: json[assetIt].partnername
        };

        asset.image = utils.findImage(json[assetIt].image, 128, this.id);
        asset.largeImage = utils.findImage(json[assetIt].image, 256, this.id);
