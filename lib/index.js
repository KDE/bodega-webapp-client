...
var currentChannels = [] //the current channels in which we iterate
...
//asynchronously :)
eventEmitter.parseChannels = function(args) {
    this.emit('parseChannels', args);
}

var doWork = function(req, res) {
//    eventEmitter.addListener('parseRecursively', parseRecursively);
    app.BodegaManager.connect(options('channels', null, true), function(remoteRes, chunk) {
        ...
        eventEmitter.parseChannels(mainCategoryId);
    });
...
eventEmitter.on('parseChannels', function(categoryId) {
    //start the iteration from the rootChannels
    currentChannels = rootChannels
    while (hasMoreChannels) {
        currentChannels = parseRecursively(currentChannels, hasMoreChannels, categoryId);
            return;
    }
});

function parseRecursively(channelList, hasMoreChannels, categoryId) {
    var nextSubcategories = [];
    //now iterate in the root channels
    for (it in channelList) {
        app.BodegaManager.connect(options('channel/' + channelList[it].id, null, true), function(remRes, data) {
            ...
            if (r.assets.length > 0) {
                ...
                if (channelList[it].id === categoryId) {
