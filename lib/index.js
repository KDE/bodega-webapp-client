...
//FIXME until we find a better way
//we store the req variable into to a global one.
//
var req2;
function parseRootChannels(req, res) {
    function next() {
        ...
        req2 = req;
        app.BodegaManager.connect(options(req, 'channel/' + root.id, null, true), function(remRes, data) {
...
var doWork = function(req, res) {
  ...
  /*  rootChannels = [];
    ...
    app.BodegaManager.connect(options(req, 'channels', null, true), function(remoteRes, chunk) {
...
sockjs_echo.on('connection', function(conn) {
    conn.on('data', function(data) {
        ...
        app.BodegaManager.connect(options(req2, 'channel/' + categoryId, query, true), function(remRes, data) {
