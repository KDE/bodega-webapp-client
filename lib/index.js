        ...
        for (it in rootChannels) {
            makeRequest(options('channel' + it.id), function(r) {
                ...
                if (r.channels > 0) {
                    ...
                    //create our array
                    rootChannels[it].subcategories = []
                    for (subcategoryIt in r.channels) {
                        subcategory = {
                            id: subcategoryIt.id,
                            name: subcategoryIt.name,
                            description: subcategoryIt.description,
                            image: subcategoryIt.image //TODO  it may be null
                        };
                        rootChannels[it].subcategories.append(subcategory);
                    }
                ...
                if (r.assets > 0) {
                    ...
                    for (assetIt in r.assets) {
                        ...
                        rootChannels[it].assets.append(asset);
                    }
                }
            });
        }
