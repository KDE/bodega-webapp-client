...
var communicate = function(req, res, remoteRes, chunk) {
    ...
    if (reply.error && reply.error.type === 'PurchaseMethodMissing') {
        ...
        res.render('paymentmethod', {
            ...
            cardNumber: reply.card.last4,
            cardBillingAddress: reply.card.address_line1,
            cardCountry: reply.card.country,
            cardState: reply.card.address_state,
            cardZipCode: reply.card.address_zip
        });
    }
};
...
var requestUrl = function(req, res) {
    ...
    var options = option(req, 'participant/paymentMethod', null, true);
